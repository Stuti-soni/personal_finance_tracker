...
#[cfg(test)]
mod tests {
    use super::{FigConfig, FigData, FigSaveType};

    #[test]
    fn empty_config_defaults_to_safe_values() {
        let config = toml::from_str::<FigConfig>("").unwrap_or_default();

        assert_eq!(config.save_type(), FigSaveType::default());
        assert_eq!(config.get_character(), ("⬆".into(), "⬇".into()));
    }

    #[test]
    fn fresh_data_starts_at_zero() {
        let data = FigData::default();

        assert_eq!(data.get_balance(), 0.0);
        assert!(data.get_transactions().is_empty());
    }
}
...
pub fn set_fs() -> (FigData, FigConfig) {
    ...
    let config: FigConfig = toml::from_str(
        &fs::read_to_string(&config_file).expect("Couldn't read config file"),
    )
    .unwrap_or_default();
